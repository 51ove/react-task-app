...
function App() {
  ...
  const handleDeleteBoard = ()=>{
    ...
    if(boards.length > 1) { // boards 는 boardArray
      ...
      const newIndexToSet = ()=>{
        const indexToBeDeleted = boards.findIndex( // 삭제된 board 인덱스 찾아서 저장
          board => board.boardId === activeBoardId
        )

        return indexToBeDeleted === 0 // 삭제된 board id가 0이면 그 다음 board / 0이 아니면 그 전 board
          ? indexToBeDeleted + 1
          : indexToBeDeleted - 1;
      }

      setActiveBoardId(boards[newIndexToSet()].boardId) // 삭제된 board 대신 활성화 될 board 설정
